...
# and setup.py
if [ -n "$ver" ]; then
    perl -p -i -e "s/(?<=:Copyright: \(c\))[0-9]{4}(?= by Pablo)/$YEAR/g" \
        `find ../ -name '*.py'`
    perl -p -i -e "s/(?<=:Version: ).+/$ver/g" `find ../ -name '*.py'`
    perl -p -i -e "s/(?<=version=').+(?=',)/$ver/g" `find . -name 'setup.py'`
    echo "Headers of jgrampro files are now set at '$ver' version and the" \
        "year '$YEAR'"
else
